...
## xv6 weibo record
```
上古神书，《操作系统设计与实现》，讲minix的，
我很记得我读大学的时候在学校的小卖部兼书店仰视过。。。
想买来摆桌子，现在估计不是minix了，换作xv6
（话说回来现在xv6也有专门讲的书了） ​​​

xv6好像是MIT的人把Unix V6（作者里奇，跟golang的plan9有关）
从原版的奇怪机器架构移植到x86上，所以本体是一本书xv6-book
（在书中印代码系列），里面再指向一个仓库叫xv6-public。
我买了一本清大的xv6书，看能不能看得懂。实际上xv6-book
这本书还参考了另一本书叫《莱昂氏unix源代码分析》，
又是一本神书。其实之所以可以用于教学，我认为原因很可能
是里奇的风格是倾向于把操作系统设计得非常小以运行在一些
嵌入式设备上，所以代码量应该不会太大。另外也有人
把xv6移植到k210上

我试了一下，用ubuntu 14可以很轻松地编译和运行xv6-public
（make && make qemu），原版的README有编译的说明。
由于xv6没有图形界面，可以直接串口命令行，所以qemu会出现
两个提示符（一个在qemu窗口内，一个在make qemu所在的控制台），
而且qemu可以通过非图形界面运行，只保留make qemu所在的
那个控制台。用户态程序（诸如mkdir）处于文件系统的根，
执行ls会看到类似dos的dir的显示结果

我买了一本《Linux内核完全注释》——还没到手，就是那本可以用
VC编译的Linux源代码（误）。实际上我买这本书只是为了摆书架，
如果想研究OS还不如去研究2.4的Linux或者最新的Linux，
或者xv6也好，因为那些更实用（除了xv6，xv6简直就像个小型dos），
我最主要的目的是想看看汇编怎么写（吐槽：那为什么不买汇编的书，
因为没那么枯燥）

其实那些讲OS的书都是超乎想象的难懂，不如写个framebuffer程序那么好玩。
例如xv6，为什么没有人搞个vc版出来？而且xv6是很久以前就出现，
然而我找一下，发现很少arm移植，可见这东西非常难改。至于
《Linux内核完全注释》和《Linux 0.01内核分析与操作系统设计》这两本，
虽然代码简化了很多，但仍然有很多超纲的内容

我买的《操作系统原型——xv6分析与实验》到手了，很厚的一本。内容较多，
我对OS不了解（待细看）。这本书不是基于最新的xv6，而是比较旧的
xv6-public tag rev9，可能因为这本书成书的时候是比较早以前，
还没有稳定版的rev11。有比较详细的gdb和代码修改说明。
用centos 7和ubuntu 18编译，我之前试过ubuntu 14也是没问题的

《操作系统原型xv6分析与实验》前言和第1章读后感。编译make；
运行make qemu；调试监听make qemu-gdb（启动gdbserver）；
调试附加target remote；线程info thread；无UI运行make qemu-nox ​​​

《xv6分析与实验》第2章读后感。实验代码在xv6-exp。一些简单修改。
（1）启动信息（2）新增my-app可执行文件（仅打印）（3）新增系统
调用getcpuid（4）进程调度测试（用Ctrl+P显示进程状态）

既然我可以看免费的xv6-book（有中文翻译版），为什么还要看
《xv6分析与实验》这本呢？这两本的定位不同。xv6-book看样子
还是个草案，只有100多页（没贴源代码），如果是中文的话
可能就只有50多页。而《xv6分析与实验》这本会讲得浅一点
（不确定）——简单来说就是看不懂，所以不看也无所谓了
（最大的好处是编译非常容易）

用gdb连xv6的qemu gdbserver，感觉回到以前调试gba程序
（vba也是带了gdbserver功能），就差没把gdb图形界面搞出来
（大概可以用eclipse-cdt，我记得以前好像有个tcl/tk的
图形界面版gdb，似乎是叫Insight，已经是时泪，大概
没几个人知道），不过Android JNI（NDK）开发也可以
玩gdbserver，只不过一般都懒得用罢了

《xv6分析与实验》第3章读后感。这章是讲整个xv6的宏观架构，
例如bootloader启动过程、qemu使用到的镜像、Makefile结构、
源码结构。并且概括了四种内核对象：进程、内存、文件系统和设备，
这四种内核对象大概是如何实现的（当然这些在Linux下都是叫文件）。
后面的章节也是讲这些内容，只不过会对着代码一个个来讲

《xv6分析与实验》第4章读后感。这章讲引导部分。一堆gcc汇编，
狂喜。流程很复杂，不过有个总的流程图（我只看懂这个，害怕），
大概是bootasm.s->bootmain.s->entry.s->main.c->scheduler()。
感兴趣可以买来看。前两个文件是bootblock代码，后面是kernel代码
（可能有多cpu情况）。这章跟12章有关。这本书说要改Makefile，
加入-monitor stdio，这样就可以获取到qemu提示符，查看寄存器
内容（不解，可能是因为qemu的gdbserver操作起来比较麻烦，
要另外连target remote，这样会容易输入一些）

我去查这本书《Linux 内核设计的艺术》，然后有人说这本书不好。
我的看法是，Linux 0.11虽然很好，但文件系统镜像不好魔改，
这方面不如xv6那么容易自己魔改。。。其实套用《Linux内核
设计与实现》的翻译者所说的话：读代码固然可以开拓视野，
但写代码才能真正让人进步——所以最后还是需要自己魔改或者实现

除了《xv6分析与实验》，还有两本书，直接就是讲Unix v6，
是《Unix内核源码剖析》和《返璞归真：UNIX技术》，可能还
有更多。不过这两本都不适合研究（或者说更难看懂） ​​​

关于操作系统的研究，到目前我觉得最好的是xv6，没有之一。
其次是Linux 2.6，代码量较小（但我试过写成Makefile会非常长，
小心，未试验过）。最后才到Linux 0.11和之类的（文件系统
不知道怎么生成，苦恼），还有众多DIY OS和什么mingw、vc6编译的OS。
对于一个随时都会卡死，打印出调用栈，多线程没办法调试的OS，
我觉得研究的主要目的也只是希望能跑得动罢了，没有必要看得懂，
但要看得懂别人的修改，然后自己拿来改就好了

关于Unix的分支，大概是这样子的（不是很准确）：
Unix v6（最早公开版本）派生出xv6（后来才出现）和
bsd（大学授权，新增虚拟内存功能），Unix v7派生出Xenix（后来被卖出去）
和System V（后来还有Solaris），bsd派生出freebsd，
而freebsd派生出osx。linux和minix则不是派生自unix的代码。 ​​​

总体来说linux 0.11的软盘根文件系统最好使用bochs（虽然后来
有人搞出了支持qemu的根文件系统），那么xv6呢？xv6的qemu是双IDE硬盘，
第一个硬盘是OS镜像，第二个硬盘是根文件系统，而且根文件系统是
通过c程序生成的，比较容易修改。linux 0.11把整个gcc（古老版本）
打进去硬盘版的根文件系统中，所以只能在qemu里头自己打代码
然后编译了，不够方便

《操作系统原型 xv6分析与实验》第5章读后感。这章是内存模型。
不得不吐槽，大部分OS书都是先讲进程模型再讲内存模型，这本书反过来说。
不过内存模型比较容易理解，就是页表罢了（逃）。没看懂，从略，
因为我没学过大学的操作系统课程，不过最后有个图，
大概就是初始化、页表（和与进程空间相关的虚拟内存）、
与内存分配有关的系统调用，这些东西之间的框图。
虽然这本书没明说，不过xv6的设计实现是简化过的，
所以那些linux的进程内存模型实现只会比这更难懂

《操作系统原型 xv6分析与实验》第6章进程。PCB（进程控制块）
与调度（状态切换），上下文切换（堆栈切换），进程同步（自旋锁，没写进程通信），
exec，init，sh（shell程序），ulib（小型C运行时，用于内存管理和控制台io）。
由于这一章进程几乎就是OS的最主要内容（其次是文件系统和对应的系统调用），
所以看不懂，从略，涉及很多数据结构

《操作系统原型 xv6分析与实验》第7章读后感，中断、异常、系统调用。
这三个是用户空间调用内核功能（或者说内核提供功能）的三种方式，
其中中断和异常是类似的，中断就是ISR，被动触发的异步操作（通常是硬件设备，
键盘或硬盘），异常是被动触发的同步中断（通常是CPU错误产生的），
这两个一般需要注册函数去传入（这本书没讲？）。而系统调用syscall是
用户空间主动调用内核功能的唯一途径，通常是sys_系统调用名，并且分配
一个系统调用号。系统调用就是一个简化设计的机制，便于维持不同版本OS
的接口兼容性，题外话，linux的系统调用数量是所有unix和类unix中最少的

《操作系统原型 xv6分析与实验》第8章读后感。调度、信号量、IPC
（共享内存和消息队列）、内存分配。调度包括：时间片、优先级、调度器。
信号量和IPC包括：自己实现信号量、共享内存和消息队列（原版可能没有）。
内存分配包括：自己实现（原版可能没有），类似linux的不连续内存分配空间。从略

《操作系统原型 xv6分析与实验》第9章读后感。文件系统xv6fs（后面提到一些
第9章设备的内容）。从略。众所周知Unix的内容只有两部分，进程和文件系统（误），
不过这本书把文件系统（硬盘设备）和设备分开两章。设备（IO设备，如键盘鼠标），
通常是通过特殊的寄存器去读写，xv6的做法是不同于文件系统（待考），用不同的
方式（不同系统调用？）读写。题外话，实际上不管进程还是文件系统（设备），
实际上都可以通过汇编或寄存器方式绕过去，所以有人说OS是没必要的，不过
其实还是有一定好处，可以简化实现，提供更容易使用的接口（提供机制，但实现可以有很多）

《操作系统原型 xv6分析与实验》第10章到第12章读后感。第10章讲设备（寄存器控制），
例如终端io，串口，定时器，中断，等等。第11章讲自己实现线程库（原版可能没有），
自己实现虚拟内存（这里可能是指文件系统实现交换内存），文件权限和绕过文件系统
的raw读写（用于实现交换内存），多用户终端。最后一章12章讲x86的硬件规范

《操作系统原型 xv6分析与实验》这本书全部看完了，很厚的一本，可以照着里面的
步骤实践，我觉得值（如果自己看代码会不知道怎么改）。总体来说，xv6就是一个
实现比linux 2.6简单很多的OS，几乎所有的实现算法都不同（文件系统和内存布局
是简化过），有些syscall接口改动过（例如设备不是通过文件系统的syscall去读写），
有些内容缺了要自己补充上去（例如IPC的消息队列和线程库），有些数据结构需要自己
修改去增加功能（例如PCB和页表）。你可以认为这本书就是要读的人自己把xv6改成好像
linux那样完善的功能——虽然看上去普通人应该做不到

你肯定很好奇，既然有xv6，那有没有类似于xv6的CPU IP核大学课程？——好像还真能找到一个，
是东南大学的minisys（可能有很多版本例如minisys-1），好像基于mips——我下巴都掉下来了，
不清楚是否开源的。其他例如risc-v应该也有，只是看上去就不简单 ​​​

最近打算不买开发板了，木有钱，看看假期能不能把xv6那本书再看一遍和做一下里面的代码实验
（现在也只有那本书说xv6的），因为我之前没实际动手做，只是写一下笔记。另外可能会买一两本书，
等假期又可以写读书笔记了（其实也是mips相关的）

买了三本书，2本mips和1本linux，打算放假看，有两本到手了，一本是《Linux技术内幕》，
是xv6分析与实验那本书作者写的，说是花了三年写的。另一本是《计算机系统设计（上册）》，
是天津大学的mips的FPGA软核实现MiniMIPS32——当然这本书的下册会更进一步，
好像是说SoC的外设问题，那本我以后再买。第三本我明天再说

先说第一本，《Linux技术内幕》，这本书比xv6分析那本书更厚更大，分两部分，进程和文件系统。
这本不是教材，是纯理论讲解，没有实践内容。作者说是把自己的疑惑解答都写进去。当然这本书
并不是面向嵌入式的，例如那些网络层、设备树、显卡加速、图形接口之类的内容这本书应该是不说
或者没有详细分析的。这本书没有具体的Linux版本号，在附录里面说的是Linux-3.13，
大概是2014年。现在的内核大版本号是5了

这次假期计划，代码计划：（1）研究xv6编译调试（2）调试minimips32；
读书计划，写读后感：（1）《计算机系统设计上册》，mips书，这本的下册类似《CPU设计实战》
（2）《Linux技术内幕》，非源码研究，粗略看（3）《汇编语言程序设计》北邮版，这本较简单 

我测试过，可以在mingw上编译运行xv6。实际上我看到gh上也有人用mingw编译运行xv6，
mirokuuno/xv6-windows，不过方法可能跟我有点不一样。我的做法是用一个gh上的已经
编译好的elf交叉工具链去编译（alessandromrc/i686-elf-tools），修改Makefile即可
（类似xv6的macos编译方法）。不过其他还需要修改一些地方，例如usertests.c中的
一处指针要转换为volatile char*，mkfs.c需要替换bzero和index，
Makefile需要把dd换成busybox dd，并且在msys中放置一个busybox的windows版
（我用的是busybox-w32）

我现在可以在windows下编译运行xv6（包括boot和fs），不过需要改动一些代码。
接下来我会研究boot和fs分区是怎么编译出来的 ​​​

我试过在windows下用gdb调试xv6也是可以的，也需要修改Makefile，大概有两种方法：
（1）修改-O0后编译，然后在运行xv6之后按Ctrl P，即可输出调用栈地址，可以查kernel.asm
或者用addr2line查询这个十六进制数字是对应哪个文件哪一行
（2）或者用make qemu-gdb启动。如果不成功，可能要改Makefile，改成-gdb:tcp:127.0.0.1:25500，
然后用gdb kernel连接target remote :25500。如果想挂起qemu，让gdb不卡住，
可以点qemu菜单上的Pause暂停，那么gdb就不会卡住了。
（3）总体来说这两种方法都不太好，只是比较简单的调试方法，还是需要自己看懂代码

很想吐槽，xv6不同版本之间差异很大，例如xv6-public和xv6-riscv就已经有不同了
（见console.c）,看xv6-public和jos也不一样（jos是分lab的，更像是原版）。
xv6-public和unix v6的不同就更离谱了（吐槽：只能说是精神续作，基本上不一样）。
这里有一定原因是xv6只实现很少的系统调用，所以甚至有人想出把minix 2的代码也塞进去xv6里面
（大概把unix v6的代码塞进去xv6也可以，啊这）

xv6-public的说明指出，它是不严格地遵从unix v6的风格和结构，是重新实现，所以其实准确来说
xv6并不是完全的v6移植版——或者说只是比较像罢了（这可能跟minix的意思差不多，也是接口像）。
它明确说明代码的来源，JOS，Plan 9，FreeBSD和NetBSD

其实根本不用担心看不懂Linux的源代码，因为Linux源代码的简化版，linux-0.11（16位版8086），
unix v6（似乎可以用模拟器运行），还有xv6（套着unix v6的名字实际带BSD代码），还有minix，
这几个最简单的操作系统的代码量都是很惊人的，我觉得我随便看懂一个都足够玩很久了。
我有时间统计对比一下代码行数

我大概统计了一下这几个类unix代码的代码量，大概是这样。值得注意的是这些代码是需要花费
很多人力写出来和测试的，并非想象中只有一两个人做出来的：
...
《Linux内核精析》全书大概浏览一下。这本书有CD-ROM，不过没什么用，里面是一些
代码摘录（可能和书中的摘录相同），还有一份2.6版的内核代码。现在看内核一般的做法
是看0.11版（未跨架构之前的版本），但直接看2版本内核也有好处，毕竟稳定时间比较长，
有更多的CPU机制可以研究。而0.11可能就只是一些远古类型设备（例如键盘，软盘）可以用，
IPC和同步锁可能会没有，而且架构更像Unix。在我看来，其实难度也不会比0.11高很多，
因为0.11的代码也不是那么容易能看得懂（包括其他什么minix2，xv6，unix v6也是）

gh上有MS-DOS 6.0的代码，我查了一下，好像不仅仅是MS-DOS的代码被leak了，还有很多，
包括2000，XP，Server版都有源代码leak。我看这份代码应该是用汇编写的，那没事了，
我觉得看这个还不如看freedos的源代码，那个则是用C实现的——看freedos还不如直接看
unix和xv6的源代码——如果我会写汇编，我还用得着看吗，我自己都能做出来个DOS，
属于远古单片机开发程序员（误，DOS相当于RTOS）

我现在喜欢收集uclinux的旧版本源代码（在wiki上有说在哪里有存档，被快照下来），
我现在看不懂，但收藏总可以吧，可能以后会有用——其实我以后可能会收集其他古旧OS的源代码，
例如最短代码量的xv6，一些远古的rtos源代码——哪怕我看不懂，但不明觉厉 ​​​

01 xv6操作系统-课程简介
https://www.bilibili.com/video/BV1QS4y1z7yE/

可能这段时间不研究uclinux了，专心研究代码量较少的xv6和linux 0.01和linux 0.11，
可以跑模拟器qemu和bochs——甚至可以完全在windows下运行和编译

如果想研究软核OS，软核方面是mips，risc-v（如picorv32），或者模拟器，
OS方面有freertos，xv6和linux，或者类OS的basic和micropython。研究这方面有用吗？
我觉得是闲着没事干，不过如果很喜欢研究汇编语言，这方面是很有乐趣的。
资料比较多但研究的人比较少 ​​​

实际上xv6也可以像linux那样搞initrd，在其早期Makefile的写法是叫memfs（xv6memfs），
注释的解释是这样做可以不用另外搞一个磁盘，但缺点是没办法持久化根文件系统
（重启qemu后数据会丢失）——于是有一种移植xv6的做法就是只处理memfs的情况，
而且即便memfs也可以gdb。另外我很怀疑gdb调试xv6可不可以手工在代码里面插入断点汇编语句
（类似调试js那样加入debugger语句），不过可能没有人会这样想，
大概直接b 断点行号就可以了——反正怎么搞都要先看懂代码，没看懂的话根本无从下手

另外xv6是简化了一些很关键的问题，但我很在意——工具链怎么来。
有些人的解释是来源于crosstool-ng（大概buildroot也行？）。
所以这东西感觉还是很撞大运的，或者它刚好就适配ubuntu这个开发环境，
如果它刚好不能工作那怎么办？我觉得会很棘手 

我把mipssim（topsy的配套Java模拟器）和Topsy（一种小型mips os）
大概能稍微跑起来了（开头有输出，但很快会报错），因为有人移植到比较新的工具链，
但我还是没有完全跑起来。以后找时间改，暂时能力不够不知道怎么调试操作系统代码。
我觉得还是从xv6入手研究比较好，如果有好的突破口的话 ​​​

其实xv6有专门讲怎么做elf-gcc工具链的，而且很详细，只不过我以前没注意。
当然xv6不关心工具链，或者说它只能用特定的工具链编译。所以研究OS的最大问题是很难跨编译器，
换个编译器就编译不了和跑不动，当然这个问题不是问题，对于科研来说，反正研究的人也不会想什么可移植，
最好是不可移植 ​​​

其实龙芯竞赛貌似也不是直接就运行linux内核的（当然未必不可能），
而是去运行ucore（毕竟xv6没有正式的mips移植版）。其实更进一步，这里面似乎有一些玄机，
例如需要限制指令种类数量（剪裁过的MIPS），甚至连交叉工具链都是特制的，用clang代替gcc ​​​

除了os161和topsy（都是大端和r3000），应该还有一些古老的用于科研的MIPS操作系统没找到，
例如os161的前身似乎就是另一个很有名的操作系统nachos，而nachos又有C++和Java版
（当然你也可以认为nachos不是严格意义的操作系统，有点像用户态qemu）。
而nachos好像也是buaa os的作者以前用过的。我还找到nachos和xv6的类似pku-os，只是x86

运行pintos（PKU-OS）的简单效果。不可以用8字节(注：64位)的操作系统，编译后运行os会挂掉。
不知道怎么加载文件系统，看样子应该是可以像xv6那样，不过还没搞懂

研究xv6之前：切，只有区区1万行代码，有什么大不了，总有一天能看懂；
研究xv6之后：这写的是什么，我脑袋过载，已经没有什么可害怕的了。 ​​​

《操作系统原型——xv6分析与实验》，关于elf和linker script的内容，
我觉得还是太跳跃了，估计作者把这些内容搬到别的书讲，然后这本书就省略了。
其实xv6有些内容，对于研究过bare-metal的人来说，可以会心一笑，都是固定桥段 ​​​

关于xv6，昨天碰到几个很诡异的问题，暂时无法解决：
（1）8字节（注：64位）xubuntu编译运行会失败，但4字节（注：32位）ubuntu是正常的
（2）用gdb或者gdbtui或者gdb layout split可以正常调试kernel文件(gdb target remote :1234)，
但用eclipse cdt会出现奇怪的情况，例如没办法断在main函数入口处
（3）有些人推荐用vscode调试gdb，我装好了但还没时间试验

xv6的代码，好像有python脚本放到指定的服务器上进行自动（？）评分的
（虽然我没找到哪里有make grade，好像在另一个仓库叫xv6-labs-2020），
我表示五体投地了。。。参考这篇《MIT操作系统实验-XV6项目环境搭建》。
当然它都说是MIT做的了，这点程度应该不会太难吧 ​​​

我觉得xv6的make grade评分应该是基于xv6-riscv的，旧版的xv6-public应该没有。
另外我怀疑是类似xinu的单元测试那样做评分的，如果单元测试没通过的话就分低，
至于怎么处理qemu的输入输出就不知道了，最简单就是单纯读取qemu的输出文本
（可以用-nographic获取），然后比较一下就知道是否测试成功 ​​​

好像只有xv6、ucore和buaa os是有详细文档的，其他教学OS属于能让你看懂我想让你做什么就算我输的感觉 ​​​

想了一下，还是放弃研究xv6的gdb图形调试方法——有两个方面，首先gdb本身就不准
（后退调用栈的内容可能是凌乱的），其次图形界面工具有各种更复杂问题，
例如用vscode不知道怎么target remote。另外8字节系统无法运行xv6的问题，
似乎是因为只有最新版的xv6-public才有no-pie的编译选项（可能还有其它改动的地方），
如果是旧版本的话这个8字节机器无法正确运行在qemu上的bug没有被修复

可不可以用ghidra或者radare2调试xv6？这俩工具也不是很好。ghidra可能会加载一下符号，
但不会对比C代码（它可能会自己反编译一个新c代码的出来）。至于radare2，太高端了，
实在不会用，不过看样子应该和gdb类似，只是有较好的彩色高亮，可能比gdb还难用 ​​​

我对比xv6和其他OS书，发现一些很有趣的情况：
（1）Minix书《操作系统设计与实现》的代码量
实际上只有27000行左右，而且作者推荐看《Xinu方法》那本书。。。
（2）讲Unix V6的《莱昂氏Unix源代码分析》，跟xv6差不多也是1万行，
作者解释说是因为要让常驻内存的代码越短越好（当然这本书的代码不容易运行起来）

其实这些OS书是互补的，他们各有优点缺点，例如Minix书压根就不讲代码，专心讲算法；
莱昂氏书讲代码讲得非常详细（类似那些逐行注释），但压根跑不起来
（除非模拟器，汇编代码都是特殊机器的汇编）；
Linux 0.11那本书不说根文件系统怎么弄出来的；
xv6则是缺少一些system v和posix接口规范，要自己补充代码

不过我好像看到有些大学的xv6教程，也不是完全用qemu去调试开发的，而是单独整一个C++程序，
然后在里面实现对应功能（例如线程池，FAT文件系统之类）就可以了，这样gdb会容易很多，
如果用qemu，估计会看吐了（所以理论来说，整个windows版也没问题）

今天打印完xv6-rev9的剩余页数，剩下是一些汇编和硬件和用户层代码。一共有100页打完了，
慢慢看。其实我是推荐看xv6代码而不推荐看linux的代码，因为xv6是最短的（甚至还可以更短），
而且支持ramdisk，linux的话驱动程序函数指针绕太多了，而且语法太高端，默认不会给你ramdisk用。
我找找有没有xv6-book的资料

在ubuntu 1404上运行JOS，效果。JOS似乎是xv6的前身（相当于xv6取代了JOS），
我找到一个可以运行shell的，make qemu效果如下（解压final.tar）。JOS有个不同的地方是可以
单独运行里面的user mode程序，例如这样make run-hello，就会运行hello.c的程序 ​​​

我看过，清大的mooc os（mooc_os_lab）就是拿jos来改的（可能有不同，但风格是一样的）。
说法是，它融入了xv6和jos和os161三者的代码。当然mooc os和ucore又似乎不同。
ucore是不同的风格，可以跨机器（奇怪的是我编译不了） ​​​

买了三本书，我觉得都不错，后面细说，是这三本《TensorFlow 2 deep learning实战》、
《汇编语言编程基础基于LoongArch》，还有《UNIX xv6内核源码深入剖析》 ​​​

第三本是《UNIX xv6内核源码深入剖析》，这本可能更少人看，没趣的一本书，非常枯燥，
不过讲得极其详细。xv6是拿出一些intel的手册，给，你自己看；这本书大概就是拿出来逐个讲。
基于rev11，不过我目前研究的是rev9，可能没什么大区别 ​​​

我搞明白怎么用gdb调试xv6了，而不会看到乱了的调用栈。其实很多书都没说清楚，
包括我手头上的书。改Makefile，把-O3改成-g3 -gdwarf-2。如果是xv6-rev9，
有一个很明确的提示在注释上，但新版故意去掉了。另外gdb甚至可以调试用户空间的程序，
例如cat README，方法是file _cat，b main，然后就可以断在main函数处

Xv6 代码导读 (调试工具配置；调试系统调用执行) [南京大学2022操作系统-P18]
https://www.bilibili.com/video/BV1DY4y1a7YD/

其实看xv6的gdb后退调用栈似乎没有什么用，除了可以调试用户空间的程序比较方便以外。。。
我感觉OS和RTOS更像是一个bare模板，能接收各种的中断，能输出串口，能加载elf，
能加载文件系统，一个复杂的机制，不是用gdb的调用栈信息就能简单理解的——当然它
本身就是一个很长的程序，甚至有一部分是汇编写的，权当是一段珍贵的汇编程序，可以工作

第二次尝试用图形界面调试xv6，结果还是失败了。因为Eclipse CDT的旧版本虽然支持32位ubuntu 14，
但运行有问题，无法调试（可能是GTK的问题）。遂再次放弃。无所谓了，
虽然gdb很简陋且不好用，但其实也不太重要，关键是找到断点的位置，而非工具好不好用。
xv6对于我而言，不过是一个更长一点的bare程序罢了，类似这样的代码还有很多

想买块Galileo Gen2开发板耍耍，不过发现软件支持很少——应该不支持xv6的。
属于Intel的Quark SoC，架构是i586，但已经停产了。暂时找得到软件支持的有Linux、
Buildroot和Xinu，除非自己写代码，但从我目前的开发水平看，应该是做不到的。
这个板的1代需要转接才能串口控制台，建议用2代。本质是linux用lsz命令接收elf文件执行

说真的，xv6的底层IO驱动有点复杂（当然它最后是跑qemu的），不过我现在有很多xv6的资料，
感觉没那么困难了。原来xv6的用意是要求自己查intel的硬件资料，显然是会太难了。
例如键盘输入驱动，VGA输出，串口输入输出驱动，IDE硬盘写入，还有printf之类。
这么说来，我研究其他xv6的移植代码，不就可以拿到很多硬件的qemu bare-metal驱动代码？感觉是赚了

做Linux小电脑有门槛且很麻烦，目前最流行用f1c100s/f1c200s，因为这样不需要外置的
ddr 2 sdram（如mt7628），为什么要人为制造焊接的麻烦呢？大概是为了做成奇怪接口的核心板或邮票核心板。
另外linux好像有32M的最小内存足迹（不可考，例如龙芯1c，uclinux则最低可能到8M，但还是需要sdram）。
如果换做我，我把linux 0.11移植到esp32算了（有psram可以代替sdram），这样可以方便焊接，
xv6也算半个linux，不过移植就更难了。归根到底，就是大部分单片机都没有集成sdram，
所以跑linux的门槛比较高

能运行操作系统的最低配单片机是什么？我觉得是esp32的freertos（逃），
或者试试用stm32f4跑uclinux（需要sdram），或者用qemu跑xv6的内存文件系统版，
或者自己用freertos / ucos 写一个os来跑，例如stm32f103。似乎没有更便宜的方案了，
除非你自己会写一个类似于linux 0.11的东西出来，所以困难其实是怎么自己写一个OS，
因为硬件的便宜是因为软件内存需要，甚至几个k的sram都足够用

所以其实移植模拟器到ESP32有什么意义呢？它可能本身能跑xv6或者linux 0.11——
虽然很大概率是做不到。在我看来xv6和linux 0.11可能比linux本身还有趣，
因为linux的代码量太大了，其实根本没办法从中吸取什么营养——与其说它开源
倒不如说它类似于闭源的程度，因为里面gcc技巧太多看不懂 ​​​

BenOS这个项目，本来是另一本书的《ARM体系结构》的baremetal项目，可以运行在qemu和树莓派上，
现在又被移植到RISC-V qemu和D1s开发板上，所以感觉有点像xv6的更简单版。。。
不过可能它和xv6比会更简单一些，而且是渐进式开发。实际上gh上也有其他项目是用类似的方法，
渐进地开发出一个OS，但不如xv6那么经典

重新看了一遍xv6 book的中文翻译本，发现它讲得很难懂，而且有些地方可能翻译得不太好或者没写好
（实际对应的代码是rev7，我开始以为翻译的是rev9，最新是rev11）。其实老实说，
xv6最有趣的代码反倒是和硬件有关系的内容（例如内嵌汇编指令），至于那些inode和pcb之类的内容，
实际上是很枯燥乏味（不过很重要）

xv6（旧版）有三个主要版本，rev7，rev9和rev11。rev7（差不多2012年）可能是最早翻译的(xv6-chinese），
rev9是红色书（xv6分析与实验）的参考代码，rev11是白色书（xv6内核源码深入剖析）的参考代码。
这么看来rev7的代码量是最小的，但rev9和rev11的代码风格和严谨度会更好，其实代码量差别不会太大 ​​​

之前说xv6的三个版本的区别（rev7,rev9和rev11），但其实还有一个很重要的区别，
rev9有一些便于调试的开关-g3 -gdwarf-2注释在rev11中被（故意）移除，所以最好的版本估计就
只有rev9这个中间过渡版（不过我不是很确定） ​​​

上周周末我看了一下xv6-book rev9版，把rev7版中文版的页行数（它是四位数字，
前两位是页码，后两位行号，因为刚好每页就是100行）换成rev9版（对照英文原版）的行号。。。
不得不说xv6-public的代码也是很鬼斧神工来着，虽然代码量似乎不多（其实很多），
但很精妙，完全可以基于它开发一个可用的OS（据说是有基于xv6的商业OS），
我觉得最难移植的是虚拟内存和elf执行，写这个代码的人功力应该非常了得的（甚至自带了一个bootloader）

我发现ucore（x86版）可能比xv6-public更好用，因为ucore连怎么调试qemu都写出来了，
例如这篇文章《清华大学 - 操作系统 课程笔记》（作者NgCafai），怎么用Eclipse调试，
怎么看函数调用。。。不过qemu本身也可以调试，利用monitor窗口，使用info命令之类（类似gdb），
或者通过参数输出文件。。。我觉得ucore可能和xv6是共通方法的

如果没有明说，我猜很多人都会认为Linux 0.11和Linux 0.12是简化版的Linux（我就是），
其实Linux 0.11是跑在16位机上（类似DOS），和Linux是两回事。例如xv6就不是16位的 ​​​

《Linux源码趣读》第一部分，引导代码。这本书分5部分，boot，init（main），
process，shell（before），shell（after）。引导部分都是艰涩难懂的汇编，这本书主要说引导，
内存分页，中断方面的问题——但感觉没有xv6那么简洁？这本书还是很好的，但应该不是那种跳跃思维的书，
或者它可能不会从设计思路方面去讲解

我认为设计操作系统是最困难的，写出来倒不是难事（我理解FreeRTOS也是OS的一种），例如xv6，其实还可以更精简，
但xv6保留了很多实用的功能，例如shell执行命令，极大地变成一个真正可以用的项目——所以Linux 0.11可能还不如xv6，
因为你不知道怎么扩展它的功能，或者说扩展能力有限 ​​​

《Linux源码趣读》这本书粗略看完了，作者应该是比较过xv6和linux 0.11的代码，用比较易懂的方式讲解。
但其他写OS书的人通常做法是搬出一个真实可用的OS代码，然后挖一个小部分来说，相当于降维打击。。。
最明显的是《操作系统设计：Xinu方法》和《操作系统设计与实现》，甚至可以拿来真实使用。
究竟是喜欢理论派还是实用派，这取决于读的人想要怎样的体验了，通常实用的OS你会觉得书讲的内容根本不够用

随便找了一份arm版的xv6，可以在ubuntu 1404 32位下编译运行，我用的是AnvithShetty10/xv6_rpi3_port，
qemu的目标机器是arm1176（树莓派1？对应ARM11吧），直接加载内核文件（没有bootloader，qemu做bootloader），
且很可能是initramfs内存文件系统（我测试过根文件系统是可读写且读写正确）。水平还不够，暂时无法理解xv6，
我连最基本的xv6 rev9都没理解透彻，只是试试

有一个教程叫《从0手写自己的Linux x86操作系统》，它的项目名称叫diy-x86os，可以找得到，可能会缺一些文件。
不过我认为不一定要看懂这个，应该不会比xv6简单多少，看懂xv6我觉得已经够用了，这个diy-x86os源代码可能还是太难了

关于x653/xv6-riscv-fpga，2023年的时候有人用此项目成功在FPGA上用RISC-V软核（RV32ia zicsr）运行xv6。
虽然还有Linux和一些RTOS也支持risc-v，不过xv6原本不是适配RV32的，这里似乎改动过，而且xv6原本基于qemu模拟运行，
其实可以简化（qemu过于重量级），所以这个项目有研究的价值，待考未实际试验 ​​​

有一个开源的8位OS，Zeal 8-bit OS（Zeal8bit/Zeal-8-bit-OS，可能是纯汇编实现的，用z88dk-z80asm编译），
可以运行在浏览器上（Zeal8bit/Zeal-WebEmulator）。我觉得提示符可能有点类似于unix的风格（类似xv6那样） ​​​

今天入手第二本书是《基于RISC-V代理内核》，也是不太推荐。这本书没明确说项目是哪个，应该就是指
gitee的hustos/riscv-pke。你可以把risc-v pk当作linux内核或者当作xv6来看待，只不过qemu和xv6可能不太方便，
但spike pk则可以更方便调用用户态代码编译出的elf。至于pk和xv6哪个更好，哪个的代码更少更容易理解，
可能这个pk会更小型，因为它可能是渐进式补充代码的

其实我还真看到有一个可以用qemu跑起来的rv32版xv6，不过那个项目似乎改动非常大，
我怀疑除非rv32和rv有什么技巧可以互换，否则没那么容易就能跑成功的 ​​​

事实证明，RISC-V的书不用买，买了也看不懂（和Linux源码阅读书和xv6书一样的道理），
自己找一个东西做出来可能比这些都容易。主要指令集体系结构和OS的门槛比较高，
它和硬件设计和verilog都是拒人千里的，没有较好的方法论，非常灵活，但我坚信是有的，
只是我暂时还没找到，也没人愿意整理，然而我希望能整理出来。ai也是，没人会整理方法，只能靠自己研究

好像研究vf2（visionfive2）开发板的开源项目很少（其实这个开发板都出了2年了，只是最近比较便宜），
很多都是OS相关的，例如移植xv6，其他好玩容易理解的可能就只有ppsspp了，不过我还没打算研究，
因为可能要自己编译ffmpeg。其实还有一种玩法是当作开发板来研究它linux内核编译配置，
例如驱动屏幕之类（它也支持buildroot）。我再想想还有没有别的好玩的玩法

转，研究这个应该比xv6难，虽然似乎也能找到源代码，我应该不会那么早研究这个 // 自制 PDP-11 运行 2.11 BSD Unix 系统实机演示
